...
#include "shared.h"
#include <stdio.h>
#include "esp_heap_caps.h"   // ROM loaded straight from SD via fopen into PSRAM
                             // (no odroid_sdcard/overlay dependency)
...
int load_rom (char *filename)
{
  ...
  /* ROM buffer in PSRAM (2MB max); read straight from the SD card via fopen. */
  cart.rom = (uint8 *)heap_caps_malloc(0x200000, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!cart.rom) { printf("load_rom: cart.rom (2MB PSRAM) alloc failed\n"); return 0; }

  size_t actual_size = 0;
  FILE *fp = fopen(filename, "rb");
  if (fp)
  {
      actual_size = fread(cart.rom, 1, 0x200000, fp);
      fclose(fp);
  }
