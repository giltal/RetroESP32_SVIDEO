...
IRAM_ATTR void render_line(int line)
{
  ...
  else
  {
    ...
    if (vdp.reg[1] & 0x40)
    {
      ...
      {
        /* Game Gear: the VDP renders the full 256-wide plane, but the real GG LCD only shows the
           160px window at x=48..207. Without overscan the off-window columns would otherwise display
           the live scrolling background (visible as the left/right edge "drawing as you play" on our
           256-wide composite). Mask them to the backdrop, matching the real LCD bezel. */
        memset(linebuf, BACKDROP_COLOR, bitmap.viewport.x);
        memset(linebuf + bitmap.viewport.x + bitmap.viewport.w, BACKDROP_COLOR, bitmap.viewport.x);
      }
    }
    else
    {
...
void palette_sync(int index)
{
  ...
  /* Store native RGB565. (The original byte-swap was for the odroid SPI LCD's big-endian
     order; our only consumer is render_copy_palette -> the composite encoder, which wants
     native order. The pixel[]->framebuffer remap path that used the swap is #if 0'd out.) */
  pixel[index] = color;
}
...
/* ---- Game Gear on-the-fly upscale ----
 * GG's real LCD is the 160x144 window; natively it uses a small centered part of the composite.
 * When enabled (by sms_run for GG only; SMS keeps the plain copy), scale that window up to fill
 * the screen width directly here in the line copy - no extra buffer, no separate blit pass.
 * The core already emits 8-bit palette indices, so this is a pure byte copy (cheap). */
int    gg_scale = 0;          /* 0 for SMS, 1 for GG (set via gg_set_scale) */
uint8 *gg_fb = 0;             /* display framebuffer base (256-wide), absolute rows */
static uint16 gg_xmap[256];   /* dest-x -> window source-x */
#define GG_SRC_W 160
#define GG_SRC_H 144
#define GG_DST_W 256          /* fill full composite width (no side bars) */
#define GG_DST_H 230          /* 1.6x -> preserves GG aspect (~5px top/bottom border) */
#define GG_DST_Y ((240 - GG_DST_H) / 2)
void gg_set_scale(uint8 *fb, int enable)
{
    gg_fb = fb;
    gg_scale = enable;
    for (int dx = 0; dx < GG_DST_W; dx++) gg_xmap[dx] = (uint16)((dx * GG_SRC_W) / GG_DST_W);
}
...
static inline void remap_8_to_16(int line)
{
    ...
    if (gg_scale)
    {
        /* 'line' is the GG source window line (0..143). H-scale the 160px window to full width
         * into the first dest row, then memcpy it to the rows that vertical scaling duplicates. */
        const uint8 *src = internal_buffer + bitmap.viewport.x;     /* skip left border -> window */
        int d0 = (line * GG_DST_H) / GG_SRC_H;
        int d1 = ((line + 1) * GG_DST_H) / GG_SRC_H;
        uint8 *first = gg_fb + (GG_DST_Y + d0) * 256;
        for (int dx = 0; dx < GG_DST_W; dx++) first[dx] = src[gg_xmap[dx]];
        for (int d = d0 + 1; d < d1; d++)
            memcpy(gg_fb + (GG_DST_Y + d) * 256, first, GG_DST_W);
        return;
    }
