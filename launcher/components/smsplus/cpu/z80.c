...
/* Use the precomputed const flag tables (in flash) instead of malloc'ing 256KB of
   internal DRAM at runtime - that alloc fails on this tight-RAM build, leaving
   SZHVC_add/sub NULL and faulting the Z80 on the first ADD/SUB. */
#if 0
...
#if 0  /* flag tables are precomputed const (see top of file); no runtime build needed */
