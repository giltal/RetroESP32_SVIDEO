...
// Cache volume/backlight so the launcher's per-loop getters don't churn NVS every
// iteration (that constant nvs_open/close was wasted work; -1 = not yet loaded).
static int32_t s_volCache = -1;
static int32_t s_blCache  = -1;

int32_t odroid_settings_Volume_get()
{
    if (s_volCache >= 0) return s_volCache;
...
{
    if (s_blCache >= 0) return s_blCache;
    // TODO: Move to header
    int result = 2;
