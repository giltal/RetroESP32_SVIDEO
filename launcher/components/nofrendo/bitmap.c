...
/* hardware bitmap that keeps an overdraw margin (external buffer must be at least
   (width + 2*overdraw) * height bytes) */
bitmap_t *bmp_createhw_od(uint8 *addr, int width, int height, int overdraw)
{
