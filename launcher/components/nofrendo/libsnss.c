...
{
   /* SNSS_FILE is ~30KB+; allocate from PSRAM so it doesn't fail in the tight
   ** internal heap during a game (free() handles either heap region). */
   *snssFile = heap_caps_malloc(sizeof(SNSS_FILE), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
   if (NULL == *snssFile)
