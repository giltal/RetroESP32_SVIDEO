...
/* Like bmp_createhw but keeps an overdraw margin (wraps an external buffer that has room
   for width + 2*overdraw per line). Used to render NES into the shared screen buffer. */
extern bitmap_t *bmp_createhw_od(uint8 *addr, int width, int height, int overdraw);
