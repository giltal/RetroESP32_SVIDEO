...
/* If set (by the composite front-end), primary_buffer wraps this shared screen buffer
   instead of malloc'ing - so NES shares one framebuffer with the other cores. */
uint8 *nofrendo_extfb = NULL;
...
//      bmp_destroy(&back_buffer);
   ...
   if (nofrendo_extfb)
      primary_buffer = bmp_createhw_od(nofrendo_extfb, width, height, 8);
   else
      primary_buffer = bmp_create(width, height, 8); /* 8px overdraw */
   if (NULL == primary_buffer)
      return -1;
