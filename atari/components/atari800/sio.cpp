...
/* PSRAM disk cache: avoid SD card reads during emulation (conflicts with I2S DMA) */
#ifdef ESP_PLATFORM
static uint8_t *disk_cache[SIO_MAX_DRIVES] = { NULL, NULL };
static long    disk_cache_size[SIO_MAX_DRIVES] = { 0, 0 };
#endif
...
#ifdef ESP_PLATFORM
	/* Cache entire disk image in PSRAM to avoid SD card reads during emulation.
	   I2S DMA (audio) conflicts with SDMMC causing read timeouts. */
	if (disk[diskno - 1] != NULL) {
		long cur = ftell(disk[diskno - 1]);
		fseek(disk[diskno - 1], 0, SEEK_END);
		long fsize = ftell(disk[diskno - 1]);
		fseek(disk[diskno - 1], 0, SEEK_SET);
		uint8_t *cache = (uint8_t *)heap_caps_malloc(fsize, MALLOC_CAP_SPIRAM);
		if (cache) {
			size_t rd = fread(cache, 1, fsize, disk[diskno - 1]);
			if ((long)rd == fsize) {
				/* Cache loaded — close real file to free SD card */
				fclose(disk[diskno - 1]);
				disk[diskno - 1] = fmemopen(cache, fsize, "rb");
				if (disk[diskno - 1]) {
					disk_cache[diskno - 1] = cache;
					disk_cache_size[diskno - 1] = fsize;
					printf("SIO: Cached disk %d in PSRAM (%ld bytes)\n", diskno, fsize);
				} else {
					/* fmemopen failed — re-open from SD as fallback */
					free(cache);
					disk[diskno - 1] = fopen(SIO_filename[diskno - 1], "rb");
					printf("SIO: fmemopen failed, falling back to SD\n");
				}
			} else {
				free(cache);
				fseek(disk[diskno - 1], cur, SEEK_SET);
				printf("SIO: Cache read incomplete (%d/%ld)\n", (int)rd, fsize);
			}
		} else {
			printf("SIO: No PSRAM for disk cache (%ld bytes)\n", fsize);
		}
	}
#endif
...
#endif
#ifdef ESP_PLATFORM
		if (disk_cache[diskno - 1]) {
			heap_caps_free(disk_cache[diskno - 1]);
			disk_cache[diskno - 1] = NULL;
			disk_cache_size[diskno - 1] = 0;
		}
#endif
