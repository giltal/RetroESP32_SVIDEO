...
/* Disable the YPOS_BREAK_FLICKER debug feature — it fills scanlines with
   0x0F when a break-ypos is hit, which should never trigger in normal play
   but we compile it out to be safe and save a tiny bit of code. */
...
/* Disabled — we run NTSC mode only */
/* #define PAL_BLENDING 1 */

/* Compile out the simple PAL blending code in ANTIC_Frame that reads back
   and blends adjacent scanlines in Screen_atari — we run NTSC only and
   this eliminates a post-render pass over the screen buffer. */
...
/* Disabled for ESP-IDF / Odroid Go — no host serial R: device */
//#define R_SERIAL 1
...
/* #undef SERIO_SOUND */
/* Disabled for performance — removes per-POKEY-write sampbuf overhead */
