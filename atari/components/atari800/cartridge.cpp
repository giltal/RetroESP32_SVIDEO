...
#include "cartridge.h"
#include "memory.h"
/* ESP-IDF: removed #include "..\emu.h" (Arduino project header) */
...
/* Forward declarations for map_file/unmap_file (defined below InsertCartridge) */
...
/* ESP-IDF: map_file/unmap_file - simple heap-based ROM loading for Odroid Go */
uint8_t* map_file(const char* path, int len)
{
    printf("map_file: %s (%d bytes)\n", path, len);
    FILE *f = fopen(path, "rb");
    if (!f) {
        printf("map_file: failed to open %s\n", path);
        return NULL;
    }
    uint8_t *buf = (uint8_t *)heap_caps_malloc(len, MALLOC_CAP_SPIRAM);
    if (!buf) {
        printf("map_file: failed to allocate %d bytes\n", len);
        fclose(f);
        return NULL;
    }
    ...
    fclose(f);
    if (rd < len) {
        printf("map_file: short read %d/%d\n", rd, len);
    }
    return buf;
}
