...
static uint8_t *binload_cache = NULL;
static long binload_cache_size = 0;
#endif
...
int BINLOAD_Loader(const char *filename)
{
	...
	printf("BINLOAD_Loader: fopen('%s') = %p\n", filename, BINLOAD_bin_file);
...
#ifdef ESP_PLATFORM
	/* Cache the XEX file in PSRAM to avoid SD card reads during emulation */
	{
		fseek(BINLOAD_bin_file, 0, SEEK_END);
		long fsize = ftell(BINLOAD_bin_file);
		fseek(BINLOAD_bin_file, 0, SEEK_SET);
		uint8_t *cache = (uint8_t *)heap_caps_malloc(fsize, MALLOC_CAP_SPIRAM);
		if (cache) {
			size_t rd = fread(cache, 1, fsize, BINLOAD_bin_file);
			if ((long)rd == fsize) {
				fclose(BINLOAD_bin_file);
				BINLOAD_bin_file = fmemopen(cache, fsize, "rb");
				if (BINLOAD_bin_file) {
					if (binload_cache) heap_caps_free(binload_cache);
					binload_cache = cache;
					binload_cache_size = fsize;
					printf("BINLOAD: Cached XEX in PSRAM (%ld bytes)\n", fsize);
				} else {
					free(cache);
					BINLOAD_bin_file = fopen(filename, "rb");
					printf("BINLOAD: fmemopen failed, falling back to SD\n");
				}
			} else {
				free(cache);
				fseek(BINLOAD_bin_file, 0, SEEK_SET);
				printf("BINLOAD: Cache read incomplete\n");
			}
		} else {
			printf("BINLOAD: No PSRAM for cache (%ld bytes)\n", fsize);
		}
	}
#endif
	...
	if (SIO_drive_status[0] == SIO_NO_DISK)
		SIO_DisableDrive(1);
	if (fread(buf, 1, 2, BINLOAD_bin_file) == 2) {
		printf("BINLOAD_Loader: first 2 bytes = %02X %02X\n", buf[0], buf[1]);
		if (buf[0] == 0xff && buf[1] == 0xff) {
			BINLOAD_start_binloading = TRUE; /* force SIO to call BINLOAD_LoaderStart at boot */
			printf("BINLOAD_Loader: XEX header OK, coldstarting\n");
