...
// Force-load every ROM bank from SD into PSRAM now. gnuboy otherwise demand-loads banks during
// gameplay (GetRomPtr -> fread), and each SD read parks the other CPU for ~15ms, stalling the
// composite video ISR -> the whole picture rolls/shakes. Pre-loading (done before the video pump
// starts) means zero SD reads during play. One-time ~1s for a 1MB ROM.
void rom_preload_all(void)
{
	for (int b = 0; b < mbc.romsize; b++)
		(void)GetRomPtr(b);
}
